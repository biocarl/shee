...
export class CounterPresenterComponent implements OnInit, PresenterView {
  ...
  ngOnInit(): void {
    this.queueService.listenToClientChannel<CounterClientSubscribeResponse>(
      (counterSubscriptionEvent) => {
        if (
          counterSubscriptionEvent.interaction &&
          counterSubscriptionEvent.interaction === 'counter'
        ) {
          this.counter++;
        }
        if (counterSubscriptionEvent.participantName) {
          console.log(
            counterSubscriptionEvent.participantName + ' is listening.'
          );
        }
      }
    );
