...
export class PollPresenterComponent implements PresenterView, OnInit {
  ...
  ngOnInit(): void {
    this.queueService.listenToClientChannel<PollClientSubscribeResponse>(
      (pollSubscriptionEvent) => {
        if (!this.questionEvent) {
          console.error(
            'Error: question event was not populated by parent client component'
          );
          return;
        }
        ...
        if (
          ...
          pollSubscriptionEvent.question_id === this.questionEvent.id
        ) {
          this.questionResponses = this.questionResponses.map(
            (total, index) => total + pollSubscriptionEvent.voting[index]
          );
        }

        if (pollSubscriptionEvent.participantName) {
          console.log(
            pollSubscriptionEvent.participantName +
              ' has voted for ' +
              this.questionEvent.answers[
                pollSubscriptionEvent.voting.indexOf(1)
              ]
          );
        }
      }
