...
export class ComponentChooserService {
  ...
  injectComponent(
    ...
    if (type != 'client' && type != 'presenter') {
      ...
      console.error(
        'Error: No matching interaction id was found for ' + interaction
      );
    }
    ...
    if (type === 'presenter') {
      switch (interaction) {
        case 'poll':
          const pollPresenterRef =
            viewContainerRef.createComponent<PollPresenterComponent>(
        ...
        case 'counter':
          const counterRef =
            viewContainerRef.createComponent<CounterPresenterComponent>(
              CounterPresenterComponent
            );
          counterRef.instance.initializeComponent(event);
          break;
        default:
          viewContainerRef.createComponent<NotFoundComponent>(
            NotFoundComponent
          );
          console.error(
            'Error: No matching interaction id was found for ' + interaction
    ...
    if (type === 'client') {
      switch (interaction) {
        case 'poll':
          const pollClientRef =
            viewContainerRef.createComponent<PollClientComponent>(
              PollClientComponent
            );
        ...
        case 'counter':
          const counterRef =
            viewContainerRef.createComponent<CounterClientComponent>(
